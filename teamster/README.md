...
| `teamster/`       | A Python directory that contains code for your Dagster repository                 |
| `teamster_tests/` | A Python directory that contains tests for `teamster`                      |
...
pyenv virtualenv $PYTHON_VERSION teamster
pyenv activate teamster
```

**Conda**
```bash
export PYTHON_VERSION=X.Y.Z
conda create --name teamster python=PYTHON_VERSION
conda activate teamster
```
...
Tests can be found in `teamster_tests` and are run with the following command:

```bash
pytest teamster_tests
```

As you create Dagster ops and graphs, add tests in `teamster_tests/` to check that your
